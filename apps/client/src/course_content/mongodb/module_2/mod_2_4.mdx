...
**Context:** Sometimes you want to count how many _unique_ categories exist in your product catalog. While `$count` counts documents, we can combine it with `$group` to count unique groups.
...
- If `$count` is the _first_ stage in a pipeline (which is rare, as you'd usually use `countDocuments()`), it can use collection statistics (metadata) for a fast count if no query predicate is present.
