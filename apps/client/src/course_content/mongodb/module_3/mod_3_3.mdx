...
_Note: Starting in MongoDB 4.2, `$set` was introduced as an alias for `$addFields`. They function identically, but `$set` is often preferred for readability as it aligns with the update operator syntax._
...
```json
[
  { "_id": 1, "item": "Apple", "price": 0.5, "qty": 10 },
  { "_id": 2, "item": "Banana", "price": 0.3, "qty": 5 }
]
```
...
```json
[
  { "_id": 1, "item": "Apple", "price": 0.5, "qty": 10, "totalCost": 5.0 },
  { "_id": 2, "item": "Banana", "price": 0.3, "qty": 5, "totalCost": 1.5 }
]
```
