...
To answer this, PostgreSQL provides the `EXPLAIN` command. This tool reveals the **Query Plan**—the step-by-step roadmap the database engine created to execute your SQL.
...
If you run `EXPLAIN ANALYZE SELECT...`, it **actually runs** the query and reports the _actual_ time taken.
...
If you ask for `SELECT * FROM books`, the database will likely choose a Sequential Scan even if you have an index. Why? Because reading the whole book is faster than looking up every single word in the index one by one. `EXPLAIN` helps you understand _why_ the database made that choice.
