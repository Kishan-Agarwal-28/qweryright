...
- **Why:** Wrapping a column in a function (like `YEAR()`) "hides" the raw data from the index. The database has to calculate the year for _every single row_ (Sequential Scan) instead of just looking up the dates in the index. This is called **SARGable** (Search ARGument able) queries.
