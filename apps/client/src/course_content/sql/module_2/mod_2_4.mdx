...
- **Example:** `WHERE NOT category = 'Romance'`
  - This is equivalent to `WHERE category != 'Romance'`. It returns everything _except_ Romance novels.
...
We want to see a list of all books that are _not_ out of stock.
...
_(Note: You could also write `WHERE stock_quantity > 0` or `WHERE stock_quantity != 0`, but `NOT` is useful for more complex logic)._
...
_Problem:_ SQL processes `AND` before `OR`. This query asks for:

1.  Mystery books under $10.
2.  OR _any_ Fiction book (regardless of price).
...
_Fix:_ Parentheses force the database to evaluate the `OR` first. "Is it Fiction or Mystery? If yes, THEN check if it is under $10."
