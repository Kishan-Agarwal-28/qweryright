...
export const Route = createFileRoute('/learning/$course/$courseId')({
  beforeLoad: ({ params }) => {
    ...
    if (!lessonData?.lesson) {
      const currentCourseStructure =
        courseStructure[courseType as 'sql' | 'mongodb'] || courseStructure.sql
  ...
  loader: ({ params }) => {
    const prev = getAdjacentLesson(params.course, params.courseId, 'prev')
    const next = getAdjacentLesson(params.course, params.courseId, 'next')
    const savedHighlights: Array<SavedHighlight> = [
...
export default function RouteComponent() {
  ...
  const mdxModules =
    courseType === 'mongodb' ? mongodbMdxModules : sqlMdxModules
