...
import { Upload, Database, Loader2, CheckCircle, AlertCircle } from 'lucide-react';
...
export function MongoDataLoader({ onLoadDefault, onLoadCustom, isLoading }: MongoDataLoaderProps) {
  ...
  const handleLoadDefault = async () => {
    try {
      ...
      if (typeof window === 'undefined' || !window.crypto || !window.indexedDB) {
        throw new Error('Your browser does not support required APIs (IndexedDB and Web Crypto). Please use a modern browser like Chrome, Firefox, or Edge.');
      }
      ...
      setStatus('error');
      setErrorMessage(error instanceof Error ? error.message : 'Failed to load default data');
    }
  };
  ...
  const handleFileSelect = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      setStatus('error');
      setErrorMessage(error instanceof Error ? error.message : 'Failed to load custom data');
    }
  };
        ...
        <div className="md:col-span-2">
          <div className="flex items-start gap-3 p-4 bg-destructive/10 border border-destructive/50 rounded-lg">
            ...
            <div className="flex-1">
              <h4 className="font-semibold text-destructive mb-1">Error Loading Data</h4>
