...
export function PageHeader({ contentRef, className }: PageHeaderProps) {
  ...
  // Helper to determine where a breadcrumb click should go (View Mode)
  const getSafePath = (accumulatedPath: string, currentSegments: string[]) => {
    if (accumulatedPath === '/learning/sql') {
      return SQL_DEFAULT_PATH;
    }
    if (accumulatedPath === '/learning/mongodb') {
      return MONGODB_DEFAULT_PATH;
    }
    if (accumulatedPath === '/learning') {
      if (currentSegments.includes('mongodb')) {
        return MONGODB_DEFAULT_PATH;
      }
      return SQL_DEFAULT_PATH;
    }
  ...
  return (
    <div className={cn("flex flex-col gap-6 pb-8 border-b border-border/40", className)}>
                  ...
                  <React.Fragment key={rawPath}>
                    <BreadcrumbItem>
                      {isLast ? (
                        ...
                        <BreadcrumbLink asChild>
                          <Link to={safePath}>{formattedSegment}</Link>
                        </BreadcrumbLink>
                      )}
                    </BreadcrumbItem>
        ...
        <div className="flex items-center gap-2">
          ...
          <TooltipProvider>
            ...
            <Popover open={isShareOpen} onOpenChange={setIsShareOpen}>
              ...
              <PopoverContent align="end" className="w-auto p-0 border-none bg-transparent shadow-none">
                 <ShareContent 
                    url={typeof window !== 'undefined' ? window.location.href : ''} 
                    meta={{ title: typeof document !== 'undefined' ? document.title : 'Course Lesson', description: "Check this course out!" }} 
