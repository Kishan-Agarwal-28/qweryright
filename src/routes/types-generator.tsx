    ...
    if (tableMatch) {
      ...
      currentInterface = tableMatch[1];
        ...
        if (colMatch) {
            ...
            let tsType = "any";
            if (rawType.match(/INT|SERIAL|DECIMAL|NUMERIC|FLOAT|REAL|DOUBLE/)) tsType = "number";
            ...
            const isNullable = !line.toUpperCase().includes("NOT NULL") && !line.toUpperCase().includes("PRIMARY KEY");
            const optionalFlag = (config.makeOptional || isNullable) ? "?" : "";
            const nullUnion = (!config.makeOptional && isNullable) ? " | null" : "";
            const finalName = applyCasing(rawName, config.casing === "pascal" ? "camel" : config.casing);

            fields.push(`  ${finalName}${optionalFlag}: ${tsType}${nullUnion}${config.semicolons ? ";" : ""}`);
        }
    }
  });
...
const parseSqlToZod = (sql: string, config: GeneratorConfig): string => {
  ...
  flushSchema();
  return output === `import { z } from "zod";\n\n` ? "// No valid CREATE TABLE statement found." : output;
};
  ...
  try {
    ...
    const generate = (obj: any, indentLevel = 1): string => {
      ...
      for (const key in obj) {
        const value = obj[key];
        const newKey = applyCasing(key, config.casing === "pascal" ? "camel" : config.casing);
        ...
        lines.push(`${indent}${newKey}${optional}: ${typeStr}${config.semicolons ? ";" : ""}`);
      }
      return lines.join("\n");
    };
    ...
    <div className="h-[calc(100vh-4rem)] flex flex-col bg-background/95">
      ...
      <div className="border-b px-6 py-3 flex items-center justify-between bg-background shrink-0">
        <div className="flex items-center gap-4">
            ...
            <div>
                <h1 className="text-lg font-semibold leading-none">Types Generator</h1>
                <p className="text-xs text-muted-foreground mt-1">Convert Database Schema & JSON to Interfaces</p>
            </div>
        </div>
        ...
        <div className="flex flex-col border-r bg-muted/10 h-full overflow-y-auto">
             ...
             <div className="p-4 flex flex-col gap-6">
                ...
                <div className="space-y-3">
                    <Label className="text-xs uppercase text-muted-foreground font-bold">Output Target</Label>
                    <Select 
                        value={outputFormat} 
                ...
                {/* Casing */}
                <div className="space-y-3">
                    <Label className="text-xs uppercase text-muted-foreground font-bold">Key Casing</Label>
                    <Select 
                        value={config.casing} 
                        onValueChange={(v: any) => setConfig(prev => ({...prev, casing: v}))}
                    >
                        <SelectTrigger>
                            <SelectValue />
                        </SelectTrigger>
                ...
                <div className="space-y-4 pt-2">
                    <Label className="text-xs uppercase text-muted-foreground font-bold">Formatting</Label>
                    
                    <div className="flex items-center justify-between">
                        <Label htmlFor="opt-semi" className="text-sm font-normal">Semicolons</Label>
                        <Switch 
                            id="opt-semi"
                            checked={config.semicolons}
                            onCheckedChange={(c) => setConfig(prev => ({...prev, semicolons: c}))}
                        />
                    </div>

                    <div className="flex items-center justify-between">
                        <Label htmlFor="opt-optional" className="text-sm font-normal">Make all Optional</Label>
                        <Switch 
                            id="opt-optional"
                            checked={config.makeOptional}
                            onCheckedChange={(c) => setConfig(prev => ({...prev, makeOptional: c}))}
                        />
                    </div>
                </div>

                <div className="pt-4 mt-auto">
                    <Button className="w-full bg-primary/10 text-primary hover:bg-primary/20 cursor-default" variant="ghost">
        ...
        <div className="flex flex-col h-full overflow-hidden bg-muted/5">
            <div className="px-4 py-2 border-b flex justify-between items-center text-xs font-mono text-muted-foreground bg-background">
                ...
                <Button 
                    ...
                    size="sm" 
                    className={cn("h-7 px-3 text-xs gap-2 transition-all", isCopied && "bg-green-600 hover:bg-green-700 text-white")}
                    onClick={handleCopy}
                >
