...
import { Query, QueryType, Difficulty, getQueriesForTimer } from "@/pages/query-writer/data/queries";
...
const getLeadingWhitespaceLength = (text: string, startIndex: number): number => {
  let count = 0;
...
export const useTypingTest = (
  ...
  const initializeTest = useCallback(() => {
    ...
    const queries = getQueriesForTimer(queryType, timerDuration, difficultyFilter);
  ...
  const handleKeyDown = useCallback(
    (e: React.KeyboardEvent<HTMLDivElement>) => {
      ...
      if (!state.isActive && e.key.length === 1) {
        ...
        timerRef.current = window.setInterval(() => {
          setState((prev) => {
            ...
            if (prev.startTime) {
              const correctChars = prev.charStates.filter((s) => s === "correct").length;
              const incorrectChars = prev.charStates.filter((s) => s === "incorrect").length;
              const totalTyped = correctChars + incorrectChars;
              const elapsedMinutes = elapsedSeconds / 60;
              const wpm = elapsedMinutes > 0 ? Math.round(correctChars / 5 / elapsedMinutes) : 0;
              const raw = elapsedMinutes > 0 ? Math.round(totalTyped / 5 / elapsedMinutes) : 0;
      ...
      if (e.key === "Enter") {
        ...
        if (expectedChar === "\n") {
          setState((prev) => {
            ...
            const whitespaceLength = getLeadingWhitespaceLength(fullText, newIndex);
      ...
      setState((prev) => {
        const newCharStates = [...prev.charStates];
        newCharStates[prev.currentCharIndex] = isCorrect ? "correct" : "incorrect";
        const newStats = calculateStats(
          newCharStates,
          prev.startTime,
          Date.now()
