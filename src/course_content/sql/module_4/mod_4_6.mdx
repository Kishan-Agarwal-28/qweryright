...
Sometimes, a single query isn't enough to answer a question. You might need to calculate a value first (like an average) and *then* use that value to filter your data.
...
```
*   *Note:* While this works, a `JOIN` is often more efficient for this specific task. Subqueries are best when you are aggregating data (like Scenario 1).
...
### Scenario 3: Authors with Above-Average Book Prices *for their Genre*
This is complex. We want to find books that are expensive *compared to other books in the same genre*. A $20 comic book is expensive; a $20 textbook is cheap. We can't just compare to the global average.
...
*   **How it works:**
    ...
    2.  It pauses and runs the inner query. The inner query calculates the average price of *only* "Sci-Fi" books (because `WHERE b2.genre = b1.genre`). Let's say the Sci-Fi average is $12.
