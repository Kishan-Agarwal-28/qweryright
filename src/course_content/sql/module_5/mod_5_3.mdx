...
3.  **`DENSE_RANK()`**: Assigns a rank, and handles ties *without* skipping numbers. (e.g., 1, 2, 2, 3).
...
### Scenario 2: Top Selling Book *Per Category*
We want to find the #1 most expensive book in *each* category. We combine ranking with `PARTITION BY`.
