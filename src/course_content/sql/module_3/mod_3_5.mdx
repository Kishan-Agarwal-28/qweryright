...
*(Note: In a real normalized database, we would join `Order_Items` to `Books`. For this module, let's assume we have a denormalized view or we are just looking at `Books` inventory value as a proxy, or we will use a simplified `Sales` table for demonstration).*
