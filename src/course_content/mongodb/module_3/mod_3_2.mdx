...
```json
[
  { "_id": 1, "amount": 50, "customerName": "Alice Smith", "customerEmail": "alice@example.com" },
  { "_id": 2, "amount": 120, "customerName": "Bob Jones", "customerEmail": "bob@example.com" }
]
```
...
- **Pipeline Order:** Filter your input documents with `$match` *before* the `$lookup` stage to minimize the number of lookups performed.
