...
[
  ...
  { "_id": 3, "reportId": "ERROR-0905", "message": "Database connection failed" }
]
```

## Exercises

1. **Weekend Traffic:** Write a pipeline to count how many logs were generated on a weekend (Saturday or Sunday). *Hint: Use `$dayOfWeek`.*
2. **Tag Count:** Add a field `tagCount` that shows how many tags are attached to each log entry. *Hint: Use `$size`.*
