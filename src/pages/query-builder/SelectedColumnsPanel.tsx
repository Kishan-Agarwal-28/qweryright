...
export default function SelectedColumnsPanel({ columns, onChange, onColumnClick }: SelectedColumnsPanelProps) {
  ...
  if (columns.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-16 px-4 text-center">
        ...
        <div className="text-sm font-medium text-foreground mb-1">No columns selected</div>
        <div className="text-xs text-muted-foreground max-w-[220px] leading-relaxed">
          Click on columns in the diagram to add them to your query. Right-click for more options.
        </div>
      </div>
    );
  }
        ...
        >
          ...
          <div className="flex items-center gap-2 mb-3">
            ...
            <div 
              ...
              onClick={(e) => onColumnClick(col.table, col.column, e.clientX, e.clientY)}
            >
          ...
          <div className="flex items-center gap-2">
            ...
            >
              <SelectTrigger className={cn(
                "h-8 flex-1 text-xs",
                col.aggregate ? "border-qb-aggregate/50 text-qb-aggregate" : 
                col.window ? "border-qb-window/50 text-qb-window" : ""
              )}>
            ...
            <Button
              ...
              onClick={() => {
                const nextSort = !col.sort ? 'ASC' : col.sort === 'ASC' ? 'DESC' : undefined;
                updateColumn(idx, { 
                  sort: nextSort,
            ...
            >
              ...
              <span className="text-xs font-mono font-semibold">{col.sort || '—'}</span>
            </Button>
          </div>
            ...
            <div className="flex flex-wrap gap-1.5 mt-3 pt-3 border-t border-border">
              ...
              {col.having && (
                <div className="inline-flex items-center gap-1 text-[10px] bg-qb-output/15 text-qb-output px-2 py-1 rounded-full">
                  <Filter className="w-2.5 h-2.5" />
                  <span className="font-mono">
                    HAVING {col.having.operator} {col.having.value?.substring(0, 10)}
                  </span>
                  <Button
                    variant="ghost"
                    size="sm"
