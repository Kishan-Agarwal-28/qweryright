...
import { Plus, X, Database, ChevronDown, ChevronRight, Copy } from 'lucide-react';
...
interface CTEBuilderPanelProps {
  ...
  onCTEsChange: (ctes: CTEDefinition[]) => void;
  onSubqueriesChange: (subqueries: SubqueryDefinition[]) => void;
}
...
export default function CTEBuilderPanel({
  ...
  const updateCTE = (id: string, updates: Partial<CTEDefinition>) => {
    onCTEsChange(ctes.map(cte => cte.id === id ? { ...cte, ...updates } : cte));
  };
  ...
  const updateSubquery = (id: string, updates: Partial<SubqueryDefinition>) => {
    onSubqueriesChange(subqueries.map(sq => sq.id === id ? { ...sq, ...updates } : sq));
  };
  ...
  return (
    <div className="space-y-6">
              ...
              >
                ...
                {expandedCTEs.has(cte.id) && (
                  <div className="p-3 space-y-3 border-t border-border">
                    <div className="space-y-2">
                      <Label className="text-xs text-muted-foreground">CTE Name</Label>
                      <Input
                        value={cte.name}
                    ...
                    <div className="flex items-center justify-between">
                      <div className="space-y-0.5">
                        ...
                        <p className="text-[10px] text-muted-foreground">Enable for hierarchical queries</p>
                      </div>
                      <Switch
                        checked={cte.recursive || false}
                        onCheckedChange={checked => updateCTE(cte.id, { recursive: checked })}
                      />
                    </div>

                    <div className="p-2 bg-secondary/30 rounded-lg">
                      <p className="text-[10px] text-muted-foreground">
                        Select columns in the main diagram to define the CTE query.
                        The CTE will be generated from the current column selection.
                      </p>
                    </div>
                  </div>
                )}
            ...
            {subqueries.map(sq => (
              ...
              >
                ...
                {expandedSubqueries.has(sq.id) && (
                  <div className="p-3 space-y-3 border-t border-border">
                    <div className="space-y-2">
                      <Label className="text-xs text-muted-foreground">Subquery Name</Label>
                      <Input
                        value={sq.name}
                        onChange={e => updateSubquery(sq.id, { name: e.target.value })}
                    ...
                    <div className="flex items-center justify-between">
                      <div className="space-y-0.5">
                        ...
                        <p className="text-[10px] text-muted-foreground">Join subquery as a table in FROM clause</p>
                      </div>
                      <Switch
                        checked={sq.asTable || false}
                        onCheckedChange={checked => updateSubquery(sq.id, { asTable: checked })}
                      />
                    </div>

                    <div className="p-2 bg-secondary/30 rounded-lg">
                      <p className="text-[10px] text-muted-foreground">
                        Use this subquery in filter conditions with IN, EXISTS, or NOT EXISTS operators.
                      </p>
                    </div>
                  </div>
                )}
      ...
      <div className="p-4 bg-secondary/30 rounded-xl space-y-2 border border-border">
        <h4 className="text-xs font-semibold text-foreground">Query Optimization Tips</h4>
