...
import { ParsedColumn } from '@/lib/mongo-query-engine';
...
export default memo(function CollectionNode({ data }: NodeProps<CollectionNodeData>) {
  const [searchQuery, setSearchQuery] = useState('');
  const Icon = iconMap[data.styles?.icon || 'database'] || Leaf;
  const headerColor = data.styles?.color || '#10b981';
  
  const handleAction = (e: React.MouseEvent, type: string, fieldName: string, fieldType?: string) => {
    e.stopPropagation();
    window.dispatchEvent(new CustomEvent('column-action', { 
      detail: { type, table: data.label, column: fieldName, columnType: fieldType, x: e.clientX, y: e.clientY } 
    }));
  };
  ...
  const filteredFields = useMemo(() => {
    ...
    return data.columns.filter(field => 
    ...
    <div className="min-w-[320px] bg-card rounded-xl border border-border shadow-2xl overflow-hidden font-mono text-sm animate-fade-in">
      ...
      >
        ...
        <div className="flex-1">
          <div className="flex items-center gap-2">
            ...
            <span className="text-[10px] text-muted-foreground uppercase tracking-wider">Collection</span>
          </div>
      ...
      <div className="py-1 flex flex-col max-h-[400px] overflow-y-auto">
        {filteredFields.map((field: ParsedColumn) => {
          const state = field.state || {} as NonNullable<ParsedColumn['state']>;
          const isSelected = state.selected ?? false;
          const hasModifiers = state.aggregate || state.accumulator || state.sort || state.filter || state.filterGroup || state.having;
          ...
          return (
            <div 
              ...
              onContextMenu={(e) => { e.preventDefault(); handleAction(e, 'context', field.name, field.type); }}
            >
              <Handle 
                type="target" 
