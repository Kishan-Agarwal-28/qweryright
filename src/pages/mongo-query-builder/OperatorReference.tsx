...
import { ChevronDown, ChevronRight, BookOpen, Calculator, Type, Calendar, GitCompare, Braces, List } from 'lucide-react';
      ...
      <div className="p-4 space-y-4">
            ...
            <div className="rounded-xl border border-border overflow-hidden">
              <CollapsibleTrigger className="w-full flex items-center justify-between p-3 bg-secondary/30 hover:bg-secondary/50 transition-colors">
                <div className="flex items-center gap-2">
                  {openSections[category.id] ? <ChevronDown className="w-4 h-4" /> : <ChevronRight className="w-4 h-4" />}
                  ...
                  {category.operators.map((op, idx) => (
                    <div key={idx} className="p-2.5 rounded-lg bg-card border border-border space-y-1.5">
                      <div className="flex items-center justify-between">
                        <span className={`font-mono text-sm font-medium ${category.color}`}>{op.value}</span>
                      ...
                      <div className="mt-1.5 p-2 rounded bg-secondary/50 border border-border">
                        <code className="text-[10px] font-mono text-foreground/90">{op.example}</code>
                      </div>
                    </div>
                  ))}
                </div>
        ...
        <div className="mt-6 p-3 rounded-lg bg-secondary/30 border border-border">
          <p className="text-xs text-muted-foreground leading-relaxed">
            💡 <span className="font-medium">Tip:</span> These operators can be used in computed fields, custom $match expressions, and aggregation pipelines. 
            Click any example to see the syntax.
          </p>
        </div>
      </div>
    </ScrollArea>
