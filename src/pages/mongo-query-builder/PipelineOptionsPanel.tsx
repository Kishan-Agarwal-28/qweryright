import { Settings2, ToggleLeft, ToggleRight, Link2, Leaf, HardDrive } from 'lucide-react';
...
export default function PipelineOptionsPanel({ options, onChange }: PipelineOptionsPanelProps) {
    ...
    <div className="space-y-5">
      ...
      >
        <div>
          <div className="text-sm font-medium flex items-center gap-2">
            <span>Distinct</span>
            <span className="text-[10px] font-mono text-muted-foreground">$group by all</span>
          </div>
          <div className="text-xs text-muted-foreground">Remove duplicate documents</div>
        </div>
        {options.distinct ? (
      ...
      <div className="space-y-2">
        <div className="flex items-center gap-2">
          ...
          <Label className="text-xs text-muted-foreground uppercase">$lookup Behavior</Label>
        </div>
        <div className="grid grid-cols-1 gap-2">
          {LOOKUP_TYPES.map(lt => (
            ...
            >
              ...
              <div className="text-[10px] text-muted-foreground mt-0.5">{lt.description}</div>
            </button>
          ))}
        </div>
      </div>
      ...
      <div 
        ...
        onClick={() => onChange({ ...options, allowDiskUse: !options.allowDiskUse })}
      >
        <div>
          <div className="text-sm font-medium flex items-center gap-2">
          ...
          <div className="text-xs text-muted-foreground">For large aggregations exceeding RAM</div>
        </div>
        {options.allowDiskUse ? (
      ...
      <div className="grid grid-cols-2 gap-3">
        <div className="space-y-1.5">
          <Label className="text-xs text-muted-foreground uppercase font-mono">$limit</Label>
          <Input
            type="number"
            value={options.limit}
        ...
        <div className="space-y-1.5">
          <Label className="text-xs text-muted-foreground uppercase font-mono">$skip</Label>
          <Input
            type="number"
      ...
      {/* $sample */}
      <div className="space-y-1.5">
        <Label className="text-xs text-muted-foreground uppercase font-mono">$sample size</Label>
        <Input
          type="number"
          value={options.sampleSize || ''}
          onChange={e => onChange({ ...options, sampleSize: parseInt(e.target.value) || undefined })}
      ...
      <Collapsible>
        <CollapsibleTrigger className="flex items-center gap-2 w-full text-left">
          <Label className="text-xs text-muted-foreground uppercase cursor-pointer font-mono">Custom $match (JSON)</Label>
        </CollapsibleTrigger>
        <CollapsibleContent className="mt-2">
          <Textarea
            value={options.customMatch || ''}
            onChange={e => onChange({ ...options, customMatch: e.target.value || undefined })}
      ...
      <Collapsible>
        <CollapsibleTrigger className="flex items-center gap-2 w-full text-left">
          ...
          <Label className="text-xs text-muted-foreground uppercase cursor-pointer">Quick Reference</Label>
        </CollapsibleTrigger>
